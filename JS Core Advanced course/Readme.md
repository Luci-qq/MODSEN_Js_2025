...
#### Hoisting и Temporal Dead Zone (TDZ)

##### Hoisting
**Hoisting** — это механизм в JavaScript, который поднимает объявления переменных и функций в верхнюю область видимости перед выполнением кода. Это означает, что вы можете использовать переменные и функции до их фактического объявления в коде.
##### 
**Temporal Dead Zone (TDZ)** — это область времени, в которой переменные, объявленные с помощью `let` и `const`, существуют, но не могут быть использованы. Это предотвращает доступ к ним до того момента, когда они были объявлены.
